{
 ...
 TLegend* legEvtClass = new TLegend(0.32,0.65,0.85,0.85);
 legEvtClass->SetLineWidth(0.0);
 legEvtClass->SetFillColorAlpha(0,0);

 TLegend* legFitCos = new TLegend(0.6,0.35,0.89,0.65);
 legFitCos->SetLineWidth(0.0);
 legFitCos->SetFillColorAlpha(0,0);


 for(int i=0;i<nsig;i++){
	hSig[i]->GetYaxis()->SetTitleOffset(0.93);
	hSig[i]->Draw();
	fFit[i]->Draw("same");
	fFitBkg[i]->Draw("same");
	fFitRho[i]->Draw("same");
	...
	legEvtClass->SetHeader("#scale[1.2]{ALICE Preliminary}","C");
	legEvtClass->AddEntry( (TObject*)0, "p#font[122]{-}Pb, #sqrt{s_{NN}} = 5.02 TeV","");
	legEvtClass->AddEntry( (TObject*)0, Form("%.0lf#font[122]{-}%.0lf%%, %.1lf < #it{p}_{T} < %.1lf GeV/#it{c}",multmin[i],multmax[i],ptmin[i],ptmax[i]),"" );
	legEvtClass->AddEntry( (TObject*)0, "f_{0}(980) #rightarrow #pi^{+}#pi^{-}, -0.5<y<0","");
